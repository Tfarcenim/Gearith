...
public class ThunderClubEntityRenderer extends EntityRenderer<ThunderClubEntity, ThunderClubEntityRenderState> {
    ...
    public ThunderClubEntityRenderer(EntityRendererProvider.Context context) {
        super(context);
        this.itemModelResolver = context.getItemModelResolver();
    }

    @Override
    public boolean shouldRender(ThunderClubEntity livingEntity, Frustum camera, double camX, double camY, double camZ) {
        return super.shouldRender(livingEntity, camera, camX, camY, camZ) && livingEntity.getPlayerOwner() != null;
    }

    @Override
    public void extractRenderState(ThunderClubEntity entity, ThunderClubEntityRenderState reusedState, float partialTick) {
        ...
        ItemStack itemstack = new ItemStack(MItems.THUNDER_CLUB);
    ...
    @Override
    public void submit(ThunderClubEntityRenderState renderState, PoseStack poseStack, SubmitNodeCollector nodeCollector, CameraRenderState cameraRenderState) {
        poseStack.pushPose();
        poseStack.pushPose();
    ...
    @Override
    public ThunderClubEntityRenderState createRenderState() {
        return new ThunderClubEntityRenderState();
    }

    @Override
    protected boolean affectedByCulling(ThunderClubEntity display) {
        return false;
    }
}
